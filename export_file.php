...
foreach($usertblrows as $tblRows)
{
	...
	foreach($usertblrows as $usertbldata)
	{
		...
		if($guest_info['last_name'])
		{
			...
			//guest-type 区分
			include("admin/inc/main_dbcon.inc.php");
			$guest_type = $obj->GetSingleData("dev2_main.spssp_guest_type", "name","id=".$guest_info['guest_type']);
			include("admin/inc/return_dbcon.inc.php");
	...
	foreach($guest_own_info as $own_info)
	{
		...
		include("admin/inc/main_dbcon.inc.php");
		$guest_type = $obj->GetSingleData("dev2_main.spssp_guest_type", "name","id=".$own_info['guest_type']);
	...
	foreach($guest_notseat_info as $notseat_info)
	{
		...
		include("admin/inc/main_dbcon.inc.php");
		$respect = $obj->GetSingleData(" dev2_main.spssp_respect", "title","id=".$notseat_info['respect_id']);
		include("admin/inc/return_dbcon.inc.php");
		$value = chop($respect);
		$html4 .= '<td>'.$value.'</td>';
		//com1 com2
		...
		include("admin/inc/main_dbcon.inc.php");
		$guest_type = $obj->GetSingleData("dev2_main.spssp_guest_type", "name","id=".$notseat_info['guest_type']);
