...
いつもお世話になっております。
 {$user_info['party_day']} {$user_info['man_lastname']}. {$user_info['woman_lastname']}様から仮発注依頼がありました。
仮発注処理をお願いいたします。
{$admin_link}
...
{$user_info['party_day']} {$user_info['man_lastname']}. {$user_info['woman_lastname']} 様から本発注依頼がありました。
本発注処理をお願いいたします。
{$admin_link}
	...
	function printCompany_upload_admin_notification_mail($user_id,$print_company_id)
	{
		...
		*/
		...
		if($staff_info['subcription_mail']=="0" && $staff_info['email'] != "")
		{
			...
			//"{$print_company_info['company_name']} Print Company has uploaded file for {$user_info['man_firstname']} {$user_info['man_lastname']}";
		
			$user_mail_body =<<<html
{$staff_info['name']} 様

いつもお世話になっております。
{$user_info['party_day']} {$user_info['man_lastname']}. {$user_info['woman_lastname']}様向け席次表印刷イメージが出来上がりました。
ご確認をお願いいたします。
...
html;
	...
	function printCompany_upload_user_notification_mail($user_id,$print_company_id)
	{
		...
		if($user_info['subcription_mail']=="0" && $user_info['mail'] != "")
		{
			...
			$user_mail_body =<<<html
新郎 {$user_info['man_lastname']} ･ 新婦 {$user_info['woman_lastname']} 様
...
html;
	...
	function process_mail_user_suborder($user_id,$print_company_id)
	{
		...
		*/
		...
		$marriage_day= $this :: japanyDateFormate($user_info['marriage_day']);
		
		$subject="［ウエディングプラス］△△ホテルより仮発注依頼がありました";//'spssp user sub-order mail';
		
		$user_suborder_mail_body =<<<html
{$print_company_info['name']}　様

いつもお世話になっております。
Hotel name より仮発注依頼がありました。
発注内容は以下のとおりです。
...
披露宴日時：{$marriage_day} {$user_info['marriage_hour']}:{$user_info['marriage_minute']}
商品区分：{$dowload_options}
商品名：{$user_plan_info['product_name']}
サイズ：{$print_size}
配置	：{$user_plan_info['print_type']}
担当者名：{$staff_info['name']} 様

以下のURLからログインし、データのダウンロード・校正紙の作成をお願いいたします。
また、校正紙を作成されましたら、同じ画面よりログインし、アップロードをお願いいたします。
・データーダウンロードURL
{$print_company_download_link}// printing company data upload download url
・データーアップロードURL
...
html;
	...
	function process_mail_user_print_request($user_id,$print_company_id)
	{
		...
		*/
		...
		$subject="［ウエディングプラス］△△ホテルより本発注依頼がありました";//'spssp user PRINTING REQUEST TO PRINT COMPANY mail';
		
		$user_suborder_mail_body =<<<html
{$print_company_info['name']} 様

いつもお世話になっております。
Hotel name より本発注依頼がありました。
発注内容は以下のとおりです。

本発注依頼書
ホテル名：hotel name
新郎名：l{$user_info['man_firstname']} {$user_info['man_lastname']} 様
新婦名：{$user_info['woman_firstname']} {$user_info['woman_lastname']} 様
披露宴日時：{$marriage_day} {$user_info['marriage_hour']}:{$user_info['marriage_minute']}
商品区分：{$dowload_options}
商品名：{$user_plan_info['product_name']}
サイズ：{$print_size}
	...
	function process_mail_user_gift_daylimit_request($user_id)
	{
		...
		*/
		...
		if($staff_info['subcription_mail']=="0" && $staff_info['email'] != "")
		{
			...
			//"ADMIN has been prossessesd gift day limit for {$user_info['man_firstname']} {$user_info['man_lastname']}";
		
			$user_mail_body =<<<html
いつもお世話になっております。
{$user_info['party_day']} {$user_info['man_lastname']}. {$user_info['woman_lastname']}ら引出物発注依頼がありました。
引出物発注処理をお願いいたします。
{$admin_link}
...
{$staff_info['name']} 様
いつもお世話になっております。
{$user_info['party_day']} {$user_info['man_lastname']}. {$user_info['woman_lastname']}様の席次表本発注締切日を過ぎています。
...
いつもお世話になっております。
{$user_info['party_day']} {$user_info['man_lastname']}. {$user_info['woman_lastname']}引出物本発注締切日を過ぎています。
