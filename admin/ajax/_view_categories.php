<?php
	require_once("file:///C|/xampp/htdocs/inc/class.dbo.php");
	include_once("file:///C|/xampp/htdocs/inc/checklogin.inc.php");

	$obj = new DBO();
	$post = $obj->protectXSS($_POST);
	$user_id = (int)$post['user_id'];
	
	$categories = $obj->GetAllRowsByCondition(" spssp_guest_category "," user_id=".(int)$user_id." order by display_order desc ");
    	...
    	<table width="80%" border="0" cellspacing="1" cellpadding="3" align="center" class="data">
			...
			?>
            		<tr style="<?=$stl?>">
                        ...
                        <td><a href="javascript:void(0)" onClick="edit_cat(<?=$cat['id']?>,'<?=$cat['name']?>','<?=$cat['description']?>')">Edit</a></td>
                        <td><a href="javascript:void(0)" onClick="delete_cat(<?=$cat['id']?>)">Delete</a></td>
                    </tr>
            <?php	
				$i++;
				}
	...
	if(isset($_POST['view_sub_cats']) && $_POST['view_sub_cats'] != '')
	{
		...
		$sub_categories = $obj->GetAllRowsByCondition(" spssp_guest_sub_category "," user_id=".(int)$user_id." and category_id = $catid ");
		?>
			<p class="txt3" style="width:80%; margin:0 auto; text-align:center;background-color:#ECF4FB;">
            	<b><a href="javascript:void(0);" onClick="view_cats()"><?=$catname?></a> Sub-categories</b>
                        <input type="hidden" id="category_id" value="<?=$catid?>" />
            </p>
            <p class="txt3" style="width:80%; margin:0 auto;"><a href="javascript:void(0);" onClick="add_subcategory()">区分詳細新規登録</a></p>
			<table width="80%" border="0" cellspacing="1" cellpadding="3" align="center" class="data">
	...
	{
		$sub_categories = $obj->GetAllRowsByCondition(" spssp_guest_sub_category "," user_id=".(int)$user_id." and category_id=".(int)$post['cat_id']);
	...
	{
		$categories = $obj->GetAllRowsByCondition(" spssp_guest_category "," user_id=".(int)$user_id);
		if(!empty($categories))
		{
			foreach($categories as $cat)
			{
	...
	if(isset($_POST['accept_default']) && $_POST['accept_default'] != '')
	{
		...
		$default_cat_row['user_id'] = (int)$user_id;
