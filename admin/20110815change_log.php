...
<!-- UCHIDA EDIT 11/08/04
 ...
 -->
	 ...
	 <h2>席次表データ修正ログ</h2>
        ...
        <div class="box_table">
      		<div style="text-align:left;"><?=$data_user['man_firstname']."-".$data_user['woman_firstname']?> 様</div>
...
//					$stuff_name = $obj->GetSingleData(" spssp_admin ", "username", " id='".$row[admin_id]."'"); // UCHIDA EDIT 11/08/04
			...
			?>
                    <div  style="background:<?=$bgcolor?>">
                        <table width="1000px"  border="0" align="center" cellpadding="4" cellspacing="1">
                            <tr align="left">
                           <td  width="15%">
                            		&nbsp;<?=$row[date]?>
                            </td>
								...
								<?php
									...
									foreach($before_log_array as $key=>$value)
									{
										if($value!=$after_log_array[$key])
										{
											...
											{
												include("inc/main_dbcon.inc.php");
												$respect = $obj->GetSingleData(" wplus_main.spssp_respect ", "title", " id='".$value."'");
												include("inc/return_dbcon.inc.php");
												echo $respect."<br/>";
											}
											if($key==6)
											...
											{
												include("inc/main_dbcon.inc.php");
												$guest_type = $obj->GetSingleData(" wplus_main.spssp_guest_type ", "name", " id='".$value."'");
												include("inc/return_dbcon.inc.php");
												echo $guest_type."<br/>";
											}
											if($key==9)
