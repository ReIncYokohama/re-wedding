				...
				$item.fadeOut(function() {
					...
					if(ul_length > 0 && draggedDivId !== "#tst")
					{
						...
						if(draggedDivId !== "#tst" && dropDivHtml !== null )
						{
							...
							$.post('ajax/set_session.php', {'divitems': key+"_input",'value':groupid}, function(data) {
												
							});	
							
							$.post('ajax/set_session.php', {'divitems': draggedDivKey+"_input",'value':draggedDivId+"_"+liIdArray[1]}, function(data) {
					 ...
					 else if(ul_length <= 0 && draggedDivId !== "#tst")
					 {
						...
						$.post('ajax/set_session.php', {'divitems': draggedDivKey+"_input",'value':""}, function(data) {
					 ...
					 else if(draggedDivId === "#tst" && ul_length <= 0)
					 {
						...
						$("#tst ul li ul #"+currentLiId+" #icon_"+itemid).append("<img src='img/icon01.jpg' border='0' id='icon"+itemid+"'/>");
						$("#tst ul li ul #"+currentLiId+" #icon_"+itemid+" img").fadeIn(400);
						$("#tst ul li ul #"+currentLiId+" span a").hide(200);
						$(divid+" input").val(groupid);						
						$.post('ajax/set_session.php', {'divitems': key+"_input",'value':groupid}, function(data) {
					 ...
					 $("<ul class='gallery ui-helper-reset ui-helper-clearfix'><li class='ui-widget-content' id ='" +currentLiId+"' title='"+title+"' style='border:0'>"+$item.html()+"</li></ul>").appendTo(divid);
					...
					checkFont(divid);
						...
						$(".droppable ul li").dblclick( function () { 
							...
							$("#"+thisDivId+ " ul").remove();
							$.post('ajax/set_session.php', {'divitems': thiskey+"_input",'value':""}, function(data) {
							...
							$("#tst ul li ul li#"+thisLiId+" span").html(spanhtml);
							$("#tst ul li ul li#"+thisLiId+" span a img").slideUp(200).fadeIn(200);
							
							var itemHref = $("#tst ul li ul li#"+thisLiId+" span a").attr('href');						
							...
							$("#tst ul li ul li#"+thisLiId+" span a").attr('href',newItemHref);
						...
						var divLength = draggedDivId.length;
	...
	function resetFormItems(){
		...
		$(".droppable").each(function() {
			...
			if(ulLength > 0)
			{
				...
				$.post('ajax/set_session.php', {'divitems': "reset",'value':""}, function(data) {
												
				});	
				
				var spanhtml = $(div_id+ " ul li span").html();
				...
				$("#tst ul li ul li#"+liId+" span").html(spanhtml);
				$("#tst ul li ul li#"+liId+" span a img").slideUp(200).fadeIn(200);				
				
				var itemHref = $("#tst ul li ul li#"+liId+" span a").attr('href');
				//alert(itemHref+"LI ID"+liId);
				var divLength = div_id.length;										
				var newDivlength = itemHref.length-(divLength+3);
				var newItemHref = itemHref.substring(0,newDivlength);							
				newItemHref = newItemHref+"tst');";						
				$("#tst ul li ul li#"+liId+" span a").attr('href',newItemHref);
				
				$("#tst ul li ul li#"+liId).draggable({				
					cancel: ".dragfalse",
					revert: "invalid", // when not dropped, the item will revert back to its initial position
					helper: "clone",
					cursor: "move",
