...
<?php
	...
	?>
	&nbsp; <a href="tables.php?room_id=<?=(int)$_GET['room_id']?>">Tables</a>
	<?php
	echo $pageination;
...
?>
	...
	<table align="center" cellspacing="0" cellpadding="3" border="0" class="grid" >
    	<tr class="head">
            ...
            <td valign="middle"  width="20%"> View</td> 
    ...
    <?php	
		...
		$query_string="SELECT t.*, rm.name as room FROM $table t left outer join spssp_room rm on t.room_id=rm.id where t.room_id=".$roomid." ORDER BY display_order DESC LIMIT ".((int)($current_page-1)*$data_per_page).",".((int)$data_per_page).";";
	...
	?>
		<tr>
            ...
            <td>
				...
				{
				?>
                <a href="default_plan.php?page=<?=(int)$_GET['page']?>&plan_id=<?=$row['id']?>"> Make Plan</a>
                <?php
				}
				?>
            </td>
